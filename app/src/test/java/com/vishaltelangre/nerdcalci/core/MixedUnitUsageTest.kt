...
class MixedUnitUsageTest {
    ...
    fun `basic mixed unit addition fails`() {
        testCalculate("10m + 10kg", "10m - 5s") { result ->
            assertError("Cannot add Meter and Kilogram: dimension mismatch", result, 0)
            assertError("Cannot subtract Meter and Second: dimension mismatch", result, 1)
        }
    }
    ...
    fun `unitless mixed with unit fails`() {
        testCalculate(
            ...
            assertError("Cannot add Meter and unitless number: dimension mismatch", result, 0)
            assertError("Cannot add unitless number and Meter: dimension mismatch", result, 1)
            assertError("Cannot subtract Meter and unitless number: dimension mismatch", result, 2)
            assertError("Cannot subtract unitless number and Meter: dimension mismatch", result, 3)
        }
    }
    ...
    fun `block sum with mixed units fails`() {
        testCalculate("10m", "5", "sum") { result ->
            ...
            assertError("Cannot sum Meter and unitless number: dimension mismatch", result, 2)
        }
    }
    ...
    fun `block sum with different physical categories fails`() {
        testCalculate("10m", "5kg", "sum") { result ->
            ...
            assertError("Cannot sum Meter and Kilogram: dimension mismatch", result, 2)
        }
    }
    ...
    fun `block average with mixed units fails`() {
        testCalculate("10m", "5", "avg") { result ->
            assertEquals("10.0 m", result[0].result)
            assertEquals("5.0", result[1].result)
            assertError("Cannot average Meter and unitless number: dimension mismatch", result, 2)
        }
    }
    ...
    fun `compound assignments fail on mismatch`() {
        testCalculate("x = 10m", "x += 5") { result ->
            assertError("Cannot add Meter and unitless number: dimension mismatch", result, 1)
        }
    }
    ...
    fun `complex mixed expression with variables fails`() {
        testCalculate("a = 10m", "b = 5", "a + b") { result ->
            assertError("Cannot add Meter and unitless number: dimension mismatch", result, 2)
        }
    }
